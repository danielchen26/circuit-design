...
sol, ts = run_prob_1bit(;init_relax = 2500., duration=270.,relax=2500.,signal=20.,K=0.011,n=2.81, up= 1.5);
py = plot(sol, vars = [:m1_HKCI,:m1_PhlF],
          lw = 1.5, xlabel = "time", ylabel = "concentration",
          title = "Signal Duration: 270", ylims = (0.,3*up))
##
plot(sol,vars = [:m1_HKCI,:m1_PhlF],lw = 1.5, xlabel = "time", ylabel = "concentration",title = "Signal Duration: 270",ylims = (0.,3*up))
scatter!(ts,ones(length(ts)))
ones(length(ts))
ts
...
##
anim = @animate for up = 0.1:0.1:2
    # up = 1.5
    sol, ts = run_prob_2bits(;init_relax = 800., duration=270.,relax=800.,signal=20,K=0.081,n=2.81, up= up,cycle = 10)
...
sol, ts = run_prob_2bits(;init_relax = 800., duration=270.,relax=800.,signal=20,K=0.081,n=2.81, up= up,cycle = 20)
function Carrying_bit(sol, ts)
    ts_ID = []
    C1 = []
    for ti = 1:2:length(ts)
...
# Identify the T0 for multibit counter
T0 = MB_T0(ts_ID, C1,up) # if something goes wrong, this may give me 0
...
@time @showprogress for K = 0.001: 0.02:0.1, n = 1.5:0.1:4., δ = 250:5:350, A = 20., up = 1:0.1:3 #@showprogress
    # solve DiffEq given parameters
    sol, ts = run_prob_2bits(;init_relax = 1000., duration=δ, relax=1000., signal=A, K=K, n=n, up = up, cycle = 10)
    # Get cost for this parameters set
    costtot = cost_bit2(sol, ts, up)
    println("K:$K n:$n, δ:$δ, A:$A, up:$up\n")
    @show costtot
    costtot == 8 ? push!(df2, [K, n, δ, A, up]) : nothing
...
## Plots
function plot3bits(sol, param)
    up = 1.5
    Ylim = 3*up
    # plt_opt = [legend = :topright, legendfontsize = 7, ylims =(0.,Ylim)]
    C_plt = plot(sol, vars = [:g3], legend = false, ylims =(0.,Ylim))
    C2_plt = plot(sol, vars = [:g26], legend = false, ylims =(0.,Ylim))
    B1_plt = plot(sol, vars = [:m1_HKCI, :m1_PhlF],legend = false, ylims =(0.,Ylim))
    B2_plt = plot(sol, vars = [:m2_HKCI, :m2_PhlF], legend = false, ylims =(0.,Ylim))
    B3_plt = plot(sol, vars = [:m3_HKCI, :m3_PhlF],legend = false, ylims =(0.,Ylim))
    Bit2_plt = plot(B1_plt,B2_plt,B3_plt,layout = (3,1),size=(600,500),show = true, title = "delta,  A, K,  n: $param" )
end
