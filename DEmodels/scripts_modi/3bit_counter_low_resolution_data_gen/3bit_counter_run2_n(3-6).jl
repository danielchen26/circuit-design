...
tt = []
@time @showprogress for K = 0.011: 0.02:0.1, n = 3.1:0.1:6., δ = 250:5:350, A = 20., up = 1:0.1:3 # up resolution 0.1
    @show K, n, δ, A, up, δ
    Δ0 = 20000.
    # solve DiffEq given parameters
    sol, ts = run_prob_3bits(;init_relax = 20000.,duration=δ, relax=20000., signal=A, K=K, n=n, up = up, cycle = 20)
...
##
df3
CSV.write("3Bits_DB_new2.csv", df3) # 14543x3




## some test for large n 
error_df  = DataFrame(K = Float64[], n = Float64[], δ =  Float64[],  up = Float64[])
for K=0.091, n = 7.1:0.1:10., δ = 250:10:350, up = 1:0.1:3
    @show K, n, δ, up
    param = [K, n, δ, up]
    try 
        sol, ts = run_prob_3bits(;init_relax = 20000.,duration=δ, relax=20000., signal=20, K=0.091, n=n, up = up, cycle = 20)
        # plt = plot_3bit(sol, ts, 2.5)
        # display(plt)
        Δ0 = 20000.
        costtot = cost_bit3(sol, Δ0, ts, up)
