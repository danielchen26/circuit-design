...
tt = []
@time @showprogress for K = 0.011: 0.02:0.1, n = 1.5:0.1:3., δ = 250:5:400, A = 20., up = 1:0.2:3 # up resolution 0.1
    @show K, n, δ, A, up, δ
    Δ0 = 20000.
    # solve DiffEq given parameters
    sol, ts = run_prob_3bits(;init_relax = 20000.,duration=δ, relax=20000., signal=A, K=K, n=n, up = up, cycle = 20)
...
@variables m4_LexA1(t) m4_IcaR(t) m4_CI1(t) m4_PsrA(t) m4_BM3RI(t) m4_HKCI(t) m4_PhlF(t)
D = Differential(t)
...
eqs4 = [
    ...
    D(g33)     ~ ξ * hill(g31 + g32)           - deg(g33), # g3 sserves as the input for the 2nd bit
    # 2nd AND gate combines (out g23,p)
    D(g34)     ~ ξ * hill(p)                   - deg(g34),
    D(g35)     ~ ξ * hill(g33)                 - deg(g35),
    D(g36)     ~ ξ * hill(g34 + g35)           - deg(g36), # g3 sserves as the input for the 2nd bit
...
de4 = ODESystem(eqs4, t,
                [m1_LexA1, m1_IcaR, m1_CI1, m1_PsrA, m1_BM3RI, m1_HKCI,m1_PhlF, g1, g2, g3,
                 ...
                 m4_LexA1, m4_IcaR, m4_CI1, m4_PsrA, m4_BM3RI, m4_HKCI, m4_PhlF], [up,dn,K,n,γ,ξ,p])
ode_f4 = ODEFunction(de4)
...
Δ0, Δ, δ, cycle, A, tspan, time, signal, ts, cb, p = init_control(Δ0 = 20000., Δ = 20000., δ = 270, cycle = 20)
param = [1.5,0.002,0.081,2.81,0.025,0.025,p]
prob0 = ODEProblem(ode_f4, u0, tspan, param)
sol = solve(prob0, Tsit5(), callback = cb, tstops = ts, reltol = 1e-13, abstol = 1e-16)

## Wrap the above problem to a fucntion
function run_prob_4bits(;init_relax,duration,relax,signal,K,n,up,cycle)
    u0 =  Float64[i for i in rand(1:22, 43)]
    Δ0, Δ, δ, cycle, A, tspan, time, signal, ts, cb, p = init_control(Δ0 = init_relax, Δ = relax, δ = duration, A = signal, cycle = cycle)
    param = [up,0.002,K,n,0.025,0.025,p]
    prob0 = ODEProblem(ode_f4, u0, (0,init_relax), param)
    sol0 = solve(prob0, Tsit5())
    prob = ODEProblem(ode_f4, sol0[end], tspan, param)
    sol = solve(prob, Tsit5(), callback = cb, tstops = ts, reltol = 1e-13, abstol = 1e-16)
    return sol, ts
end
## ploting
function plot_4bit(sol, ts, up; opt = 'C')
    Ylim = 2*up
    if opt == 'C'
        C_plt = plot(sol, vars = [g3], label = "Connector type 1",
                    legendtitle = "Connector module",
        ...
        C3_plt = plot(sol, vars = [g36], label = "Connector type 3",
                    legendtitle = "Connector module",
                    # ylims =(0.,Ylim)
                    )
        Bit4_plt_C = plot(
...
## run some ploting
function single_run(;δ = 270)
    sol, ts = run_prob_4bits(;init_relax = 20000.,duration= δ , relax=20000., signal=20.0, K=0.081, n=2.81, up = 1.5, cycle = 20)
    # plot(sol, vars = g23)
    plt_B = plot_4bit(sol,ts,up, opt = 'B')
    plt_C = plot_4bit(sol,ts,up, opt = 'C')
    plot(plt_B,plt_C, layout = (1,2))
end
...
# for δ = 200: 270
#     u0 =  Float64[i for i in rand(1:22, 43)]
#     Δ0, Δ, δ, cycle, A, tspan, time, signal, ts, cb, p = init_control(Δ0 = 20000., Δ = 20000., δ = δ, cycle = 20)
#     param = [1.5,0.002,0.081,2.81,0.025,0.025,p]
#     prob0 = ODEProblem(ode_f4, u0, tspan, param)
#     sol = solve(prob0, Tsit5(), callback = cb, tstops = ts, reltol = 1e-13, abstol = 1e-16)

for K = 0.011: 0.02:0.1, n = 1.5:0.1:3., δ = 250:5:400, A = 20., up = 1:0.2:3 # up resolution 0.1
    @show K, n, δ, A, up
    # solve DiffEq given parameters
    sol, ts = run_prob_4bits(;init_relax = 20000.,duration=δ, relax=20000., signal=A, K=K, n=n, up = up, cycle = 20)
    plt = plot_4bit(sol, ts, 2.5,opt = 'C')
    #plot
    # plt_B = plot_4bit(sol,ts,up, opt = 'B')
    # plt_C = plot_4bit(sol,ts,up, opt = 'C')
    # plt = plot(plt_B,plt_C, layout = (1,2), title = "δ: $δ")
    # display(plt)
end
