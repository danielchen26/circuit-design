...
df2 = pd.read_csv("BESS_database/2NAND.csv")
# modify df[['Optimal Network']] column
df2 = df2.replace(r'\r\n', ',', regex=True)
df3 = pd.read_csv("BESS_database/3NAND.csv")
# modify df[['Optimal Network']] column
df3 = df3.replace(r'\r\n', ',', regex=True)
...
Num_total_fun3, Boolean_dict3 = lf.Boolean_espresso_gens(3)
convertion = dict([('Or', 'And'), ('And', 'Or')])
Database3 = []
...
NOR_df3 = pd.DataFrame({'Binary': NOR3_database[:, 0], 'Expresso_tts': np.array([i[0] for i in NOR3_database[:, 1]]), 'Num_gates': [
                       i[0] for i in NOR3_database[:, 2]], 'Optimal_net': [i[1] for i in NOR3_database[:, 2]], 'Permu_int': NOR3_database[:, 3], 'Num_in': NOR3_database[:, 4]})
NOR_df3 = NOR_df3[NOR_df3.Num_in == 3]
NOR_df3.to_csv("NOR_df3.csv", index=False)


# --------- generating 2inputs database -------
Num_total_fun2, Boolean_dict2 = lf.Boolean_espresso_gens(2)
convertion = dict([('Or', 'And'), ('And', 'Or')])
Database2 = []
...
NOR_df2 = pd.DataFrame({'Binary': NOR2_database[:, 0], 'Expresso_tts': np.array([i[0] for i in NOR2_database[:, 1]]), 'Num_gates': [
                       i[0] for i in NOR2_database[:, 2]], 'Optimal_net': [i[1] for i in NOR2_database[:, 2]], 'Permu_int': NOR2_database[:, 3], 'Num_in': NOR2_database[:, 4]})
...
# # ----- calculate number of functions distribution per gates.
