...
#      How much time to wait until the system become statble again
    ...
    if sol11[7,end] < sol11[6,end]
        ul_range = push!(ul_range, ti)
        t_range = push!(t_range, sol1.t[ti])
        t_wait = push!(t_wait, sol11.t[stable_t_ind])
    end
end
...
#   2. Relax system for 1500
    ...
    t_set = push!(t_set, t2_n)
            
    P_set = push!(P_set, p_on);     P_set = push!(P_set, p_off);
    sol_set = push!(sol_set, [i.x for i in sol_impulse.u]);     sol_set = push!(sol_set, [i.x for i in sol_relax.u]);     
end
